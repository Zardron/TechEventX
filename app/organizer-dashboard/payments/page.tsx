...
export default function PaymentsPage() {
    ...
    if (data?.data) {
        console.log('Payment data received:', {
            payoutsCount: data.data.payouts?.length || 0,
            transactionsCount: data.data.transactions?.length || 0,
            paymentRequestsCount: data.data.paymentRequests?.length || 0,
        ...
        <div className="space-y-6">
            ...
            <div className="border rounded-md bg-card overflow-hidden">
                ...
                {payoutData.payouts.length === 0 ? (
                    ...
                    <div className="divide-y">
                        {payoutData.payouts.map((payment: any) => (
                                    ...
                                    <div className="flex-1">
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            {payment.type === 'payment_request' ? (
                                                <span
                                                    className={`px-2.5 py-1 rounded-full text-xs font-medium border ${statusColors[payment.status as keyof typeof statusColors] || statusColors.pending}`}
                                                >
                                                    {payment.status}
                                                </span>
                                            ) : (
                                            ...
                                            <span className="px-2.5 py-1 rounded-full text-xs font-medium border bg-blue-500/10 text-blue-500 border-blue-500/20">
                                                {payment.type === 'payment_request' ? 'Payment Request' : 'Event Payment'}
                                            </span>
                                        </div>
                                            ...
                                            {payment.type === 'payment_request' ? (
                                                <>
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="font-semibold">{formatDateToReadable(payment.requestedAt)}</p>
                                                    </div>
                                                    {payment.processedAt && (
                                                        <div>
                                                            <p className="text-muted-foreground">Processed</p>
                                                            <p className="font-semibold">{formatDateToReadable(payment.processedAt)}</p>
                                                        </div>
                                                    )}
                                                    {payment.failureReason && (
                                                        <div>
                                                            <p className="text-muted-foreground">Failure Reason</p>
                                                            <p className="font-semibold text-red-500">{payment.failureReason}</p>
                                                        </div>
                                                    )}
                                                </>
                                            ) : (
                                                <>
                                                    <div>
                                                        ...
                                                        <p className="font-semibold">{payment.event?.title || 'N/A'}</p>
                                                    </div>
                                                    <div>
                                                        <p className="text-muted-foreground">Attendee</p>
                                                        <p className="font-semibold">{payment.attendee?.name || payment.attendee?.email || 'N/A'}</p>
                                                    </div>
                                                    <div>
                                                        <p className="text-muted-foreground">Payment Method</p>
                                                        <p className="font-semibold capitalize">{payment.paymentMethod.replace('_', ' ')}</p>
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="font-semibold">{formatDateToReadable(payment.paidAt || payment.createdAt)}</p>
                                                    </div>
                                                    {payment.booking?.ticketNumber && (
                                                        <div>
                                                            <p className="text-muted-foreground">Ticket Number</p>
                                                            <p className="font-semibold">{payment.booking.ticketNumber}</p>
                                                        </div>
                                                    )}
                                                    {payment.platformFee > 0 && (
                                                        <div>
                                                            <p className="text-muted-foreground">Platform Fee</p>
                                                            <p className="font-semibold">{formatPrice(payment.platformFee)}</p>
                                                        </div>
                                                    )}
                                                    {payment.discountAmount > 0 && (
                                                        <div>
                                                            <p className="text-muted-foreground">Discount</p>
                                                            <p className="font-semibold text-green-500">-{formatPrice(payment.discountAmount)}</p>
                                                        </div>
                                                    )}
